package com.mx.testvideo.ui.adapter
...
import com.mx.testvideo.durationFormat
import com.mx.testvideo.glide.GlideApp
import com.mx.testvideo.glide.GlideRoundTransform
...
class VideoDetailAdapter(mContext: Context, data: ArrayList<HomeBean.Issue.Item>) :
    ...
    init {
        textTypeface = Typeface.createFromAsset(App.context.assets, "fonts/FZLanTingHeiS-L-GB-Regular.TTF")
    }
    ...
    private fun setVideoDetailInfo(data: HomeBean.Issue.Item, holder: ViewHolder) {
        ...
        with(holder) {
            getView<TextView>(R.id.tv_action_favorites).setOnClickListener {
                Toast.makeText(App.context, "喜欢", Toast.LENGTH_SHORT).show()
            }
            getView<TextView>(R.id.tv_action_share).setOnClickListener {
                Toast.makeText(App.context, "分享", Toast.LENGTH_SHORT).show()
            }
            getView<TextView>(R.id.tv_action_reply).setOnClickListener {
                Toast.makeText(App.context, "评论", Toast.LENGTH_SHORT).show()
            }
        }
    }
